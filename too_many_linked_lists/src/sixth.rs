...
// Imports for common collection traits
use std::cmp::Ordering;
use std::fmt::{self, Debug};
use std::hash::{Hash, Hasher};
use std::iter::FromIterator;

// Imports for implementation details
use std::ptr::NonNull;
...
impl<T> LinkedList<T> {
    ...
    pub fn pop_front(&mut self) -> Option<T> {
        unsafe {
            // Only does something if there is a front node to pop.
            self.front.map(|node| {
                // Casting the raw pointer 'at the boundary'
                ...
                if let Some(new) = self.front {
                    // Cleanup reference to the removed node
                    (*new.as_ptr()).front = None;
                } else {
                    // If the fron is now null, then this list is now empty!
                    // Removed debug_assert! due to bad panic unwrapping
                    // i.e. we were unwrapping while in an unsafe state
                    self.back = None;
                }

                self.len -= 1;
                result
                // Box gets implicitly freed
            })
        }        
    }
