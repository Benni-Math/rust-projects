...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        let index = self.chunks.iter().enumerate().find_map(|(i, c)| {
        ...
        match index {
            ...
            None => { 
                ...
                Err(PngError::ChunkNotFound(chunk_type).to_err())
            },
        }
    }
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        self.chunks.iter().find(|c| {
            c.chunk_type().to_string() == chunk_type
        })
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        ...
        if header != Png::STANDARD_HEADER {
            return Err(PngError::NonStandardHeader.to_err());
        }

        let mut chunks = Vec::<Chunk>::new();

        // This buffer will read the chunk_length info at the top of every chunk
        let mut chunk_length = [0u8; 4];
        while reader.read_exact(&mut chunk_length).is_ok() {
            // Constructing a buffer of the correct length
            let length = u32::from_be_bytes(chunk_length) as usize;
            let mut buf = vec![0u8; length + 8];

            reader.read_exact(&mut buf)?;

            // Creating the chunk by appending chunk_length and our custom buf
            let chunk_bytes: Vec<u8> = chunk_length.iter().chain(buf.iter()).copied().collect::<Vec<u8>>();
            // Pushing onto the list of chunks
...
impl fmt::Display for Png {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Number of Chunks: {}", self.chunks.len())
    }
}
...
impl fmt::Display for PngError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::ChunkNotFound(chunk_type) => write!(f, "Chunk Not Found: {}", chunk_type),
            Self::NonStandardHeader => write!(f, "Could not convert from &[u8], non-standard header found.",),
        }
    }
}
